...
class moderateCenter {
    ...
    validateAccess() {
        try {
            ...
            if (!userRole || !userRole.toLowerCase().includes('bountycreator')) {
                this.showError('Access denied. bountycreator role required.');
                setTimeout(() => {
                    window.location.href = 'login.html';
                }, 2000);
                return;
    ...
    generateSampleData() {
        const types = ['bounty', 'Response', 'Analytics'];
...
// Initialize moderateCenter when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    window.moderateCenter = new moderateCenter();
});
...
window.downloadData = function(id) {
    window.moderateCenter.showSuccess(`Downloading data for ID: ${id}`);
};
